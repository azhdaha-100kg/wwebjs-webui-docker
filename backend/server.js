...
app.post('/session/send-voice/:sessionId', authenticateToken, upload.single('audioFile'), async (req, res) => {
    const { sessionId } = req.params;
    ...
    if (!client || !clientReadyStatus[sessionId]) {
        return res.status(400).json({
            success: false,
            error: `Session '${sessionId}' tidak ready. Pastikan session ID benar & sudah scan QR.`
        });
    }
    if (!req.file) {
        return res.status(400).json({ success: false, error: 'File audio (audioFile) wajib diupload.' });
    }
    if (!number) {
        return res.status(400).json({ success: false, error: 'Nomor tujuan (number) wajib diisi.' });
    }
 
    try {
        const chatId = normalizeToJid(number, countryCode);
        const originalName = req.file.originalname || 'voice.mp3';
        ...
        console.log(`[${sessionId}] Sending VN to ${chatId} | ${originalName} | ${mimeType} | ${req.file.size} bytes`);
 
        const msgSent = await client.sendMessage(chatId, media, {
            sendAudioAsVoice: true,
        });
 
        console.log(`[${sessionId}] VN sent OK | msgId: ${msgSent.id.id}`);
        io.to(sessionId).emit('voice_sent', { sessionId, to: chatId, filename: originalName, id: msgSent.id.id });
 
        return res.json({ success: true, message: 'Voice note berhasil dikirim!' });
 
    } catch (e) {
        console.error(`[send-voice/${sessionId}] Error:`, e.message);
